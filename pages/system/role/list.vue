  ...
  <view>
    ...
    <view class="uni-container">
      <unicloud-db
        ...
        :options="options"
        :where="where"
        field="role_id,role_name,permission{permission_id,permission_name},comment,create_date"
        page-data="replace"
        :orderby="orderby"
        :getcount="true"
          ...
          <uni-tr v-for="(item, index) in data" :key="index">
            ...
            <uni-td align="center">
              ...
              <view v-else class="uni-group">
                ...
                <button
                  @click="confirmDelete(item.role_id)"
                  class="uni-button"
                  size="mini"
                  type="warn"
                >
...
export default {
  ...
  methods: {
    onqueryload(data, ended) {
      for (var i = 0; i < data.length; i++) {
        ...
        item.permission = item.permission
          .map((pItem) => pItem.permission_name)
          .join("、");
    ...
    praseRoleArr(permission) {
      return permission
        ? permission.map((pItem) => pItem.permission_name).join("、")
        : "-";
    },
  },
};
