<template>
	<view class="container">
		...
		<view class="uni-container">
			<unicloud-db ref="udb" v-slot:default="{data, loading, error, options}" collection="question"
				field="title, content,tagID,areaID" :getone="true" :where="`_id=='${formData.id}'`" :manual="true">
				<uni-forms ref="form" :value="formData" :rules="rules" validate-trigger="submit">
					<uni-forms-item labelWid="100" name="name" label="题目标题" required>
					...
					<uni-forms-item name="tag" label="选择标签" v-show="formData.areaID !== ''">
						<uni-data-checkbox ref="checkboxRef" emptyText="该专区下暂无标签..." :multiple="true"
							v-model="formData.tagID" :where="`areaID == '${formData.areaID}'`" collection="questionTag"
							field="name as text, _id as value" />
					</uni-forms-item>
					<uni-forms-item labelWid="100" name="name" label="题目内容" required>
