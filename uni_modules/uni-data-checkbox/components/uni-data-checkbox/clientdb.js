...
const attrs = ['pageCurrent', 'pageSize', 'collection', 'action', 'field', 'getcount', 'orderby', 'where'];
...
export default {
  ...
  methods: {
    ...
    remove(id, { action, callback, confirmTitle, confirmContent } = {}) {
      if (!id || !id.length) {
        return;
      }
      uni.showModal({
    ...
    _getExec() {
      ...
      const { current, size } = this.paginationInternal;
      exec = exec
        .skip(size * (current - 1))
        .limit(size)
        .get({
